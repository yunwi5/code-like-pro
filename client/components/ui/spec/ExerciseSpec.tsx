...
const ExerciseSpec: React.FC<{ exercise: IExerciseWithId }> = ({ exercise }) => {
  ...
  return (
    <ul className="text-sm md:text-[0.95rem] mt-4 lg:mt-2 flex flex-wrap gap-x-4 gap-y-2">
      {exercise.author && <ProfileView user={exercise.author} size={'1.5rem'} className="!gap-2" />}
      ...
      <li className="flex items-center gap-1">
        <BsClock className="text-blue-500/80 text-[1.3rem]" /> {getDateFormat(exercise.createdAt)}
      </li>
      <li className="flex items-center gap-1">
        <MdReportProblem className="text-stone-500 text-[1.35rem]" /> {exercise.reports.length}{' '}
        report{exercise.reports.length !== 1 && 's'}
      </li>
    </ul>
  );
};
