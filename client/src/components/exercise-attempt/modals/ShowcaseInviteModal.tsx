...
const ShowcaseInviteModal: React.FC<Props> = ({ onClose }) => {
    const { exercise } = useExerciseAttemptCtx();
    const exerciseId = exercise?._id;

    const exerciseNameHighlight = (
        <strong className="text-main-500 font-semibold capitalize">{exercise?.name}</strong>
    );
        ...
        >
            <section className="flex flex-col text-gray-700">
                ...
                <div className="flex flex-col gap-2 px-7 py-6 text-slate-700 bg-slate-200/90">
                    ...
                    <p>
                        Please consider join our showcase page for {exerciseNameHighlight} to
                        showcase your work!
                    </p>
