...
const Rank: React.FC = () => {
    ...
    useEffect(() => {
        getRanking().then((res) => {
            if (res.ok && res.data) {
                setRanking(res.data.sort((a,b) => a.creationPoints + a.solvingPoints < b.creationPoints + b.solvingPoints
                ? 1
                : -1,));
            ...
            <table className="flex-auto">
                <tbody>
                    <tr key="RankStatistic" className="col-span-full">
                        <td>
                            <button
                                className={buttonStyle + (scoreMode === 'Overall '? ' bg-gray-300': ' bg-gray-100')}
                                onClick={() => {
                                    setRanking(
                                        () => {
                        ...
                        <td>
                            <button
                                className={buttonStyle + (scoreMode === 'Creation '? ' bg-gray-300': ' bg-gray-100')}
                                onClick={() => {
                                    setRanking(
                                        () => {
                                            return rankingArray.sort((a, b) =>
                                            a.creationPoints < b.creationPoints
                                                ? 1
                                                : -1,);
                                        });
                        ...
                        <td>
                            <button
                                className={buttonStyle + (scoreMode === 'Solving '? ' bg-gray-300': ' bg-gray-100')}
                                onClick={() => {
                                    setRanking(
                                        () => {
                                            return rankingArray.sort((a, b) =>
                                            a.solvingPoints < b.solvingPoints
                                                ? 1
                                                : -1,);
                                        });
                    ...
                    <tr className="border-b border-gray-400" key="RankHeader">
                        <td className={'font-bold text-gray-600 text-center p-2 pt-6 pr-12'}>Rank</td>
                        <td className={'font-bold text-gray-600 text-center p-2 pt-6'}>User</td>
                        <td></td>
                        <td className={'font-bold text-gray-600 text-center p-2 pt-6'}>{scoreMode}Acquired Points</td>
                    </tr>
                    {rankingArray.map((user,index)=>(
                        (index >= currentPage * numOfUsersPerPage && index < (currentPage+1) * numOfUsersPerPage) ?
                        <tr className={'border-t border-gray-300' + (index%2 === 1? ' bg-gray-100':'')} key={'Rank' + index.toString()}>
