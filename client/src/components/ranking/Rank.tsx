...
const Rank: React.FC = () => {
    ...
    function sortRank(mode: number, array : IRanking[]) {
        if (mode == 1) {
            array.sort((a, b) => a.creationPoints + a.solvingPoints < b.creationPoints + b.solvingPoints ? 1 : -1);
        }// Overall mode
        if (mode == 2) {
    ...
    return (
        <>
            ...
            <table className="flex-auto">
                <tbody>
                    <tr key="RankStatistic" className="col-span-full">
                        <td><button className={buttonStyle} onClick={() => { setRanking(rankOverall); setScoreMode('Overall '); }}>Overall</button></td>
                        <td><button className={buttonStyle} onClick={() => { setRanking(rankCreation); setScoreMode('Creation '); }}>Creation</button></td>
                        <td><button className={buttonStyle} onClick={() => { setRanking(rankSolving); setScoreMode('Solving '); }}>Solving</button></td>
                        <td className="font-bold pl-12 text-gray-800">Total {NumberHandling(rankingArray.length)} Users</td>
                    </tr>
                    {(() => {
                        let index = 1;
                        let tr = [];
                        ...
                        let cellStyle = "p-4"
                        tr.push(<tr className="border-b border-gray-400" key="RankHeader"><td className={headStyle + " pr-12"}>Rank</td><td className={headStyle}>User</td><td></td><td className={headStyle}>{scoreMode}Acquired Points</td></tr>);
                        let oddChildColor = ' bg-gray-100';
                        let evenChildColor = ''
                        for (let user of page) {
                            if (index % 2 == 0) {
                                tr.push(
                                    <tr className={"border-t border-gray-300" + oddChildColor} key={"Rank" + index.toString()}>
                                        <td className={cellStyle + " font-bold pr-10 text-gray-800"}>No.{index + currentPage * numOfUsersPerPage}</td>
                                        <td className={cellStyle}><img src={user.pictureUrl && default_profile} className="rounded-full h-10 w-10 inline ml-4" /></td>
                                        <td className="pr-64">{user.name}</td>
                                        <td className={cellStyle + " px-5 text-center"}>{
                                            (() => {
                                                if (scoreMode == 'Overall ') { return user.creationPoints + user.solvingPoints; }
                                                if (scoreMode == 'Creation ') { return user.creationPoints; }
                                                if (scoreMode == 'Solving ') { return user.solvingPoints; }
                                                return '';
                                            })()
                                        }</td>
                                    </tr>
                                ...
                                tr.push(
                                    <tr className={"border-t border-gray-300" + evenChildColor} key={"Rank" + index.toString()}>
                                        <td className={cellStyle + " font-bold pr-10 text-gray-800"}>No.{index + currentPage * numOfUsersPerPage}</td>
                                        <td className={cellStyle}><img src={user.pictureUrl && default_profile} className="rounded-full h-10 w-10 inline ml-4" /></td>
                                        <td className="pr-64">{user.name}</td>
                                        <td className={cellStyle + " px-5 text-center"}>{
            ...
            <div style={{width: 800}} className="mt-10 scale-75 inline-block item-center">
                <PageNavigation currentPage={currentPage} totalPages={maxPage} onChangePage={(newPage: number) => setPage(newPage)} />
            </div>
        </>
    );
}
